...
# Never block the UI on database startup. If Supabase is temporarily down,
# Outreach should still render and explain the degraded state.
DB_READY=False
DB_ERROR=None
try:
    db.init_db()
...
def live_clock():
    ...
    <script>
      function tick(){const now=new Date();const text=new Intl.DateTimeFormat('en-IN',{timeZone:'Asia/Kolkata',day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:true}).format(now);document.getElementById('clock').textContent=text;}
...
def page_head(title,subtitle):
    ...
    with left: st.markdown(f'<div class="page-head"><div class="page-title">{title}</div><div class="page-subtitle">{subtitle}</div></div>',unsafe_allow_html=True)
...
def safe_metrics():
    if not DB_READY:return {"total_contacts":0,"sent":0,"bounced":0,"replied":0,"daily":[]}
    try:return db.get_metrics()
    except Exception:return {"total_contacts":0,"sent":0,"bounced":0,"replied":0,"daily":[]}
...
def history_df():
    if not DB_READY:return pd.DataFrame()
    try:
...
def send_one(contact,subject,body):
    if not DB_READY:
        st.error("Outreach is connected to the interface, but the database is currently unavailable. Please try again shortly.")
        return False
    ...
    except Exception as exc: db.log_event(contact["id"],"bounced",str(exc),subject);st.error(f"Could not send: {exc}");return False
...
if not DB_READY:
    st.warning("Outreach is running, but the customer database is temporarily unavailable. The interface is loaded; data-dependent sections will appear empty until the database reconnects.")

if page=="Dashboard":
    page_head("Dashboard","A focused view of your outbound email performance.");metrics=safe_metrics();a,b,c=st.columns(3)
    ...
    with history_tab:
        ...
        if f.empty:st.info("No outbound email history yet.")
            ...
            for row_number,(_,r) in enumerate(f.sort_values("dt",ascending=False).iterrows(),1):
                cols=st.columns([0.55,1.8,1.8,2.1,1.15,1.05]);cols[0].write(row_number);cols[1].write(r.get("name","") or "—");cols[2].write(r.get("company","") or "—");cols[3].write(r.get("email","") or "—");cols[4].markdown(f'<span class="badge">{r.get("sector") or "General"}</span>',unsafe_allow_html=True);cols[5].write(relative_time(r.get("timestamp")))
        st.markdown('</div>',unsafe_allow_html=True)
        if st.button("Sync replies & bounces"):
            if SENDER_EMAIL and APP_PASSWORD:
                try:result=sender.check_replies_and_bounces(SENDER_EMAIL,APP_PASSWORD);st.success(f"Synced {result['replies_found']} replies and {result['bounces_found']} bounces.");st.rerun()
                except Exception as exc:st.error(f"Sync failed: {exc}")
            else:st.warning("Configure sender credentials first.")
    with compose_tab:
        contacts={f"{c['name']} · {c['email']}":c for c in safe_contacts()}
        if not contacts:st.info("Add a customer from Customers first.")
            ...
            if st.session_state.get("schedule_open"):
                ...
                if st.button("Confirm schedule",type="primary"):
                    if not DB_READY:st.error("Database is currently unavailable.")
                    else:
                        local=datetime.combine(d,t).replace(tzinfo=IST);db.schedule_email(contact["id"],subject,body,local.astimezone(timezone.utc).isoformat());st.session_state["schedule_open"]=False;st.success(f"Scheduled for {local.strftime('%d %b %Y · %I:%M %p')} IST.");st.rerun()
                st.markdown('</div>',unsafe_allow_html=True)
    with bulk_tab:
        ...
        if rows:
            ...
            if selected:
                ...
                if st.button(f"Send to {len(selected)} customers",type="primary",use_container_width=True):
                    if not DB_READY:st.error("Database is currently unavailable.")
                    elif not SENDER_EMAIL or not APP_PASSWORD:st.error("Configure sender credentials first.")
                    else:
                        ...
                        for i,contact in enumerate(prepared,1):
                            box.info(f"Sending {i}/{len(prepared)} · {contact['email']}")
                            ok=send_one(contact,subject,body);sent_count += 1 if ok else 0;failed += 0 if ok else 1;progress.progress(i/len(prepared))
    ...
    with add_tab:
        ...
        if st.button("Save customer",type="primary"):
            if not DB_READY:st.error("Database is currently unavailable.")
            elif not name.strip() or not email.strip() or not company.strip():st.warning("Name, email and company are required.")
        ...
        if uploaded:
            ...
            if missing:st.error("Missing required columns: "+", ".join(missing))
                ...
                if st.button("Import customers",type="primary"):
                    if not DB_READY:st.error("Database is currently unavailable.")
                    else:inserted,skipped=db.upsert_contacts(edited.to_dict("records"));st.success(f"Imported {inserted} customers · skipped {skipped} duplicates/invalid rows.")
    with list_tab:
        customers=safe_contacts()
        if not customers:st.info("No customers yet.")
            ...
            all_hist=pd.DataFrame(db.get_customer_history()) if DB_READY else pd.DataFrame();all_csv=all_hist.to_csv(index=False).encode("utf-8") if not all_hist.empty else b"";dl1,dl2=st.columns([4,1])
